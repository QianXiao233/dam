...
class DamBrain:
    ...
    def deep_think(self):
        ...
        # ============================================================
        # ✅ 安全守卫：根据水位和上涨速度决定是否需要考虑开闸
        #    · 水位 < 80           → 只有快速上涨(rate>=1.0)才可能开闸
        #    · 水位 80~100（预警区间）→ 上涨慢(rate<1.0)不开，先观察
        #    · 水位 ≥ 100          → 预测可能冲上应急，放行到候选搜索
        #    · 水位 ≥ 110          → 应急，强制开闸
        # ============================================================
        rate = self._get_rate()
        if current < WATER_WARNING:
            ...
            if rate < 1.0:
                return 0
        elif current < WATER_EMERGENCY - 10:
            # 预警区间 (80~100)：上涨慢就不开，等到接近应急再考虑
            if rate < 1.0:
                return 0
